...
const getBooksInYear= async function(req,res) {
    let booksYear = await BookModel.find({"year": 2018})
    res.send({msg: booksYear})
}

const getParticularBooks= async function(req,res) {
    let particularBook = await BookModel.find({ bookName:  /^ram/i  })
    res.send({msg: particularBook})
}

const getXINRBooks= async function(req,res) {
    let priceOfBooks = await BookModel.find({"prices.indianPrice": {$in: ["100", "200", "500"] } }  )
